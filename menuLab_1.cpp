...
void info() {
	...
	CURSOR(3, 4);
	std::cout << "Найти сумму элемнтов от х_1 до х_min\n\n";
}
...
void outputEl() {
	...
	setColor(14, 0);
	std::cout << "\n\n   Сумма элементов массива до минимального элемента " << "( "
		<< min_el << " ) = ";
	double sum = 0;
...
int setSizeArr() {
	setColor(15, 0);
	std::cout << "   Измененние размера массива" << std::endl;
	CURSOR(3, 2);
	unsigned int key;
	setColor(7, 0);
	std::cout << "   Создать массив?" << "\n\n   1 - Да   2 - Нет";
	key = _getch();
	if (key == '1') {
		LINES(1);
		...
		while (std::getline(file, line)) {
			...
			while (ss >> item) { 
				...
				try {
					...
					setColor(12, 0);
					std::cerr << "   Ошибка: элемент в строке " << i + 1<< " имеет недействительное значение и был заменен на 0. Измените элемент и откройте файл заново" << std::endl;
				}
				catch (const std::invalid_argument) {
					setColor(12, 0);
					std::cerr << "   Ошибка: недейсвительный аргумент " << std::endl;
				}
				catch (const std::out_of_range) {
					setColor(12, 0);
					std::cerr << "   Ошибка: выход за пределы размера массива" << std::endl;
				}
			}
			arr[i] = result;
			i++;
...
void replaceElFile() {
	...
	if (indicator == true) {
		...
		if (key == '1') {
			...
			std::cout << "\n\n   Введите идекс элемента котороый хотите изменить (0, 1, 2, ...)" << std::endl << "   ";
			std::cin >> replaceElIdx;
			if (replaceElIdx >= sizeArr  || replaceElIdx < 0 || std::cin.fail() || std::cin.bad()) {
				std::cerr << "   Ошибка: элемента с таким индексом не существует!" << std::endl;
				while (replaceElIdx >= sizeArr || replaceElIdx < 0 || std::cin.fail() || std::cin.bad()) {
					CURSOR(3, 6);
					SPACEBACK(30);
					std::cin.clear();
					std::cin.ignore(250, '\n');
					//std::cout << "   Введите идекс элемента котороый находтся в дипозоне от 0 до " << sizeArr << std::endl;
					std::cin >> replaceElIdx;
				}
				for (int i = 4; i < 7; i++) {
					for (int j = 0; j < 30; j++) {
			...
			std::cout << "Элеменнт с индексом ";
			setColor(10, 0);
			SPACEBACK(30);
			std::cout << replaceElIdx;
			setColor(14, 0);
			SPACEBACK(30);
			std::cout << " успешно найден => ";
			setColor(10, 0);
			SPACEBACK(30);
			...
			setColor(14, 0);
			std::cout << "\n\n   Введите новое значение для этого элемента: \n   ";
			setColor(10, 0);
			std::cout << arr[replaceElIdx];
			setColor(14, 0);
			SPACEBACK(30);
...
void menuLab_1() {
	...
	do {
		...
		Text(hdc, 50, 0, "Меню лабораторной работы №1", RGB(255, 255, 255), RGB(0, 0, 0), 25);
		...
		Text(hdc, 50, 150, "2 - Даннные из клавиатуры в файл", RGB(255, 255, 255), RGB(15, 5, 77), 25);
		Text(hdc, 50, 250, "3 - Решение со значениями из файла", RGB(255, 255, 255), RGB(15, 5, 77), 25);
		Text(hdc, 50, 300, "4 - Решение из файла с табличной формой вывода", RGB(255, 255, 255), RGB(15, 5, 77), 25);
		Text(hdc, 50, 350, "5 - Замена значения элемента в файле по индексу", RGB(255, 255, 255), RGB(15, 5, 77), 25);
		...
		switch (key) {
			...
			case '1':
				...
				setColor(7, 0);
				Text(hdc, 40, 400, "ESC - Назад", RGB(255, 255, 255), RGB(157, 56, 188), 25);
				_getch();
				break;
			case '2':
				inpFile();
			...
			case '4':
				...
				outTableGraf(arr, sizeArr, sizeArr < 10 ? sizeArr : 10, "Табличная форма вывода", "X", hdc, 1);
				setColor(7, 0);
				Text(hdc, 40, 550, "ESC - Назад", RGB(255, 255, 255), RGB(157, 56, 188), 25);
				_getch();
				break;
			case '5':
				system("cls");
