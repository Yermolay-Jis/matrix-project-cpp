...
void writeBinFile() {
	...
	setColor(7, 0);
	std::cout << "   Данные успешно занесены в файл => ";
	SPACEBACK(1);
	setColor(10, 0);
	std::cout << nameFile;
	CURSOR(0, 0);
...
void readBinFile() {
	...
	if (indicator == true) {
		...
		if (numberEl == 0) { std::cout << "Файл пуст"; }
		else { std::cout << "\n\n   Количество данных в файле: " << numberEl << std::endl;}
		...
		if (!file) {
			std::cerr << "   Критическая ошибка: не удалось вернутся в начало файла\n";
			std::cerr << "   Состояние потока eof()" << file.eof() << " fail = " << file.fail() << " bad() = " << file.bad();
		}
...
void replaceElBinFile() {
	...
	if (indicator == true) { // if file is open
		...
		if (key == '1') {
			...
			Text(hdc, 50, 17, "Массив из файла:", RGB(255, 255, 255), RGB(0, 0, 0), 15);
			CURSOR(3, 0);
			outTableGraf(arr, sizeArr, sizeArr < 10 ? sizeArr : 10, "", "i", hdc, 0);

			size_t replaceElIdx;
			CURSOR(3, 12);
			Text(hdc, 50, 150, "Введите идекс элемента котороый хотите изменить (0, 1, 2, ...)", RGB(255, 255, 255), RGB(0, 0, 0), 15);
			std::cin >> replaceElIdx;
			if (replaceElIdx >= sizeArr || replaceElIdx < 0 || std::cin.fail() || std::cin.bad()) {
				Text(hdc, 50, 200, "Ошибка: элемента с таким индексом не существует!", RGB(255, 0, 0), RGB(0, 0, 0), 15);
			...
			Text(hdc, 50, 200, "Элемент с индексом ", RGB(255, 255, 255), RGB(12, 12, 12), 15);
			Text(hdc, 185, 200, std::to_string(replaceElIdx), RGB(0, 255, 0), RGB(12, 12, 12), 15);
			Text(hdc, 200, 200, " успешно найден => ", RGB(255, 255, 255), RGB(12, 12, 12), 15);
			...
			SPACEBACK(2);
			std::cin.clear();
			Text(hdc, 50, 225, "Введите новое значение для этого элемента:", RGB(255, 255, 255), RGB(12, 12, 12), 15);
			CURSOR(3, 15);
			setColor(15, 0);
			std::cin >> newElement;
			...
			while (true) {
				try {
					...
					if (!file.read(reinterpret_cast<char*>(&item), sizeof(double))) {
						if (file.eof()) { std::cerr << "   Элемент не найден в файле" << std::endl; }
						else { std::cerr << "   Ошибка чтения из файла" << std::endl; break;  }
					}
			...
			Text(hdc, 50, 250, "Измененный массив:", RGB(255, 255, 255), RGB(12, 12, 12), 15);
			size_t n = 50;
...
void menuLab_2() {
	...
	do {
		...
		Text(hdc, 50, 0, "Меню лабораторной работы №2", RGB(255, 255, 255), RGB(0, 0, 0), 25);
		Text(hdc, 50, 50, "0 - Решение в консоли", RGB(255, 255, 255), RGB(15, 5, 77), 25);
		Text(hdc, 50, 100, "1 - Решение с табличной формой вывода", RGB(255, 255, 255), RGB(15, 5, 77), 25);
		Text(hdc, 50, 150, "2 - Даннные из клавиатуры в бинарный файл", RGB(255, 255, 255), RGB(15, 5, 77), 25);
		Text(hdc, 50, 250, "3 - Решение со значениями из бинарного файла", RGB(255, 255, 255), RGB(15, 5, 77), 25);
		Text(hdc, 50, 300, "4 - Решение из бинарного файла с табличной формой вывода", RGB(255, 255, 255), RGB(15, 5, 77), 25);
		Text(hdc, 50, 350, "5 - Замена значения элемента в бинарном файле по индексу", RGB(255, 255, 255), RGB(15, 5, 77), 25);
		Text(hdc, 50, 450, "6 - Задать размер динамического массива", RGB(255, 255, 255), RGB(15, 5, 77), 25);
		Text(hdc, 50, 500, "7 - Просмотр директории", RGB(255, 255, 255), RGB(15, 5, 77), 25);
		...
		case '1':
			...
			system("pause");
			system("cls");
			Text(hdc, 500, 25, "Табличная форма вывода", RGB(255, 255, 255), RGB(12, 12, 12), 25);
			inputEl();
			solution1();
			LINES(5);
		...
		case '4':
			...
			readBinFile();
			LINES(1);
			outTableGraf(arr, sizeArr, sizeArr < 10 ? sizeArr : 10, "Табличная форма вывода", "X", hdc, 1);
			_getch();
			break;
		case '5':
			system("cls");
